...
export const HomeScreen: React.FC<HomeScreenProps> = ({
  ...
  return (
    <SafeAreaView className="flex-1 bg-midnight-950">
      {/* Decorative Circles */}
      <View className="absolute -top-32 -right-32 w-96 h-96 bg-gold-500/5 rounded-full" style={{ zIndex: -1 }} />
      ...
      >
        ...
        <View className="flex-row items-center justify-between mb-6">
          <View className="flex-1">
            <Text className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Welcome back</Text>
            <Text className="text-slate-100 font-bold text-lg">{userFullName}</Text>
          </View>

          <View className="flex-1 items-center">
            <Text className="text-gold-500 font-extrabold text-4xl tracking-tighter">Q<Text className="text-slate-100">pay</Text></Text>
          </View>
          ...
          <View className="flex-1 items-end">
            {onLogOut ? (
              <TouchableOpacity
                ...
                className="w-10 h-10 rounded-xl bg-midnight-900 border border-midnight-800 items-center justify-center"
              >
